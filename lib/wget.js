...
function download (src, output, options, _parentEvent, redirects) {
  ...
  req = request({
    ...
    if (res.statusCode === 301 || res.statusCode === 302 || res.statusCode === 307) {
      ...
      // if the redirect location header was a path only,
      // add protocol and host
      if (res.headers.location.slice(0, 1) === '/') {
        res.headers.location = `${srcUrl.protocol}://${srcUrl.hostname}:${srcUrl.port}${res.headers.location}`
      }
